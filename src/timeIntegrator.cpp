...
void TimeIntegrator::ShowLog(DataBlock& data) {
  if (isSilent) return;
  double rawperf = (timer.seconds() - lastLog) / data.mygrid->np_int[IDIR] /
                   data.mygrid->np_int[JDIR] / data.mygrid->np_int[KDIR] / cyclePeriod *
                   idfx::psize;
#ifdef WITH_MPI
  ...
  double IrradiationOverhead;
  if (data.haveIrradiation) {
    double IrradiationCycleTime = data.irradiation->elapsedTime - lastIrradiationLog;
    IrradiationOverhead = 100.0 * IrradiationCycleTime / (timer.seconds() - lastLog);
    lastIrradiationLog = data.irradiation->elapsedTime;
  }
  ...
  if (ncycles == 0) {
    ...
    if (data.haveIrradiation) {
      idfx::cout << " | " << std::setw(col_width) << "Irradiation overhead (%)";
    }
    idfx::cout << std::endl;
  }
  ...
  if (data.haveIrradiation) {
    if (ncycles >= cyclePeriod) {
      idfx::cout << " | " << std::setw(col_width) << IrradiationOverhead;
    } else {
      idfx::cout << " | " << std::setw(col_width) << "N/A";
    }
  }
    ...
    if (imbalance > allowedImbalance) {
      ...
      for (int i = 0; i < idfx::psize; i++) {
        if (computeLogPerCore[i] / computeMean - 1 > allowedImbalance / 2 / 100) {
          idfx::cout << "+" << 100 * (computeLogPerCore[i] / computeMean - 1) << "% (proc " << i
                     << ")" << std::endl;
        }
        if (1 - computeLogPerCore[i] / computeMean > allowedImbalance / 2 / 100) {
          idfx::cout << "-" << 100 * (1 - computeLogPerCore[i] / computeMean) << "% (proc " << i
                     << ")" << std::endl;
        }
      }
  ...
  for (int stage = 0; stage < nstages; stage++) {
    ...
    if (stage == 0) {
      if (!haveFixedDt) {
        ...
        if (idfx::psize > 1) {
          MPI_SAFE_CALL(
              MPI_Iallreduce(MPI_IN_PLACE, &newdt, 1, realMPI, MPI_MIN, MPI_COMM_WORLD, &dtReduce));
        }
#endif
      }
    }
...
int64_t TimeIntegrator::GetNCycles() { return (ncycles); }
...
#endif
  if (runtimeReached) {
    idfx::cout << "TimeIntegrator:CheckForMaxRuntime: Maximum runtime reached." << std::endl;
  }
  idfx::popRegion();
