...
class FLDMatrix {
 public:
  ...
  void EnforceBoundary(int dir, BoundarySide side, RadiationBoundaryType type,
                       IdefixArray3D<real> &, bool apply_physical);
  void SetBoundaries(IdefixArray3D<real> &, bool apply_physical=false);  // Set the proper boundaries for the given array
...
private:
  ...
  bool isPeriodic; // Periodicity status of the density distribution // Alex: What do I need this for?
...
class FluxLimitedDiffusion {
 ...
 public:
  ...
  enum FluxLimiterType {one_third, Minerbo1978, LevermorePomraning1981, Kley1989, userdeffluxlimiter};

  // Handling userdef boundary & opacities
  using UserDefBoundaryFunc = void (*) (DataBlock &, int dir, BoundarySide side,
                                       const real t, IdefixArray3D<real> &arr);
  using PrototypeOpacityFuncCGS  = void (*) (DataBlock* data, IdefixArray4D<real> kappaP, IdefixArray4D<real> kappaR);
