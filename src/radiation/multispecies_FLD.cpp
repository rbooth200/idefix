...
void MultiSpeciesFLD::Init(Input &input, DataBlock *datain) {
  ...
  this->kappaP = IdefixArray4D<real> ("kappaP", num_species,
                                                  data->np_tot[KDIR],
                                                  data->np_tot[JDIR],
                                                  data->np_tot[IDIR]);

  this->kappaR = IdefixArray4D<real> ("kappaR", num_species,
                                                  data->np_tot[KDIR],
                                                  data->np_tot[JDIR],
                                                  data->np_tot[IDIR]);
  this->radZ = IdefixArray4D<real> ("RadiationZ", num_species,
                                                  data->np_tot[KDIR],
                                                  data->np_tot[JDIR],
                                                  data->np_tot[IDIR]);
  
  this->radGamma = IdefixArray4D<real> ("RadiationGamma", num_species,
                                                  data->np_tot[KDIR],
                                                  data->np_tot[JDIR],
                                                  data->np_tot[IDIR]);

  this->k_eff = IdefixArray3D<real> ("kappa_eff", data->np_tot[KDIR],
                                                  data->np_tot[JDIR],
                                                  data->np_tot[IDIR]);

  this->cV_eff = IdefixArray3D<real> ("cV_eff", data->np_tot[KDIR],
                                                data->np_tot[JDIR],
                                                data->np_tot[IDIR]);
                                                  
  this->u_eff = IdefixArray3D<real> ("u_eff", data->np_tot[KDIR],
                                              data->np_tot[JDIR],
                                              data->np_tot[IDIR]);

  this->l_eff = IdefixArray3D<real> ("l_eff", data->np_tot[KDIR],
                                              data->np_tot[JDIR],
                                              data->np_tot[IDIR]);
...
void MultiSpeciesFLD::UpdatePressure(){
...

  {
    ...
    idefix_for("UpdatePressureGas", data->beg[KDIR], data->end[KDIR],
                                    data->beg[JDIR], data->end[JDIR],
                                    data->beg[IDIR], data->end[IDIR],
        KOKKOS_LAMBDA (int k, int j, int i) {
        real rho = Vc(RHO,k,j,i);

        real newtmp = (u_eff(k,j,i) + dt*code_c*k_eff(k,j,i)*Erad(k,j,i)) / Z(0,k,j,i);
  ...
  for (int s=1; s < num_species; s++) {
    ...
    idefix_for("UpdatePressureDust", data->beg[KDIR], data->end[KDIR],
                                     data->beg[JDIR], data->end[JDIR],
                                     data->beg[IDIR], data->end[IDIR],      
      KOKKOS_LAMBDA (int k, int j, int i) {
        real rho = Vc(RHO,k,j,i);
...
void MultiSpeciesFLD::FillUtils() {
  ...
  { // Gas
    ...
    idefix_for("FillUtilsGas", kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
        real rho = Vc(RHO,k,j,i);
  ...
  idefix_for("FinishRHS", kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
