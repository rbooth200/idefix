    ...
    for(int j=0; j < dim[1]; j++)
        for (int i=0; i < dim[0]; i++)
            out << i << " " << j << " " << r[i] << " " << th[j] << " " << int(rays.type(j,i)) << " "
                << rays.index_i(j,i) << " " << rays.index_j(j,i) << " "
                << rays.start(j,i) << " " << rays.length(j, i) << " " 
                << rays.order(j, i) << " " << rays.weights(j,i) << "\n";
}
...
SphericalShortChar::SphericalShortChar(Input& input, DataBlock* datain)
 : Irradiation(input, datain),
    ...
    exp_tau("Optical Depth", data->np_tot[KDIR],data->np_tot[JDIR]+1,data->np_tot[IDIR]+1,num_bands),
    kapp("Opacity", data->np_tot[KDIR],data->np_tot[JDIR]+1,data->np_tot[IDIR]+1,num_bands),
    rho_all("Densities", num_species, data->np_tot[KDIR],data->np_tot[JDIR]+1,data->np_tot[IDIR]+1)
 {
    ...
    if (input.CheckEntry("Irradiation","flux") >= 0){
        F0 = IdefixArray1D<real> ("F0", this->num_bands) ;

        IdefixArray1D<real>::HostMirror f_host = Kokkos::create_mirror_view(this->F0) ;
        for (int i = 0; i < this->num_bands; i++)
            f_host(i) = input.Get<real>("Irradiation","flux",i) ; // in cgs units

        Kokkos::deep_copy(F0,f_host);
    } else {
        std::stringstream msg;
        msg << "Irradiation:: flux must be specified." ;
        IDEFIX_ERROR(msg);
    }
...
int SphericalShortChar::_get_cell_order(int i, int j,HostRayInfo& host_rays) {
    ...
    if (order < 0){
        if (host_rays.type(j, i) == RayInfo::RayType::RADIAL || 
            ...
            if (host_rays.type(j, i) == RayInfo::RayType::RADIAL){
                if (jj > data->beg[JDIR])
                    o2 = _get_cell_order(ii, jj-1, host_rays);
            }
            else if (host_rays.type(j, i) == RayInfo::RayType::POLOIDAL) {
                if (ii > data->beg[IDIR])
                    o2 = _get_cell_order(ii-1, jj, host_rays);
            }
...
void SphericalShortChar::compute_optical_depths() {
    ...
    for (int s=0; s < num_species; s++) {
        ...
        if (this->opacityType == OpacityType::constant) {
            ...
            idefix_for("CornerOpacity", kbeg, kend, jbeg, jend+1, ibeg, iend+1,
                KOKKOS_LAMBDA (int k, int j, int i)  {
                        ...
                        double rho = 
                            0.25*(Vc(RHO,k, j ,i) + Vc(RHO,k, j ,i-1) + 
                                  Vc(RHO,k,j-1,i) + Vc(RHO,k,j-1,i-1));
            ...
            std::stringstream msg;
            msg << "Irradiation:: Opacity (kappa) type must be constant" ;
            IDEFIX_ERROR(msg) ;
        }
    }
...
void SphericalShortChar::compute_heating_rate(){
    ...
    IdefixArray1D<double> F0 = this->F0;
    IdefixArray2D<double> kappa = this->kappa; 
    ...
    idefix_for("IrradiationHeating", kbeg, kend, jbeg, jend, ibeg, iend,
        KOKKOS_LAMBDA (int k, int j, int i)  {
            ...
            double AR[2] = {
                0.5*re[ i ]*re[ i ]*(phie[k+1]-phie[k])*(sm2-sp2),
            ...
            for (int l=0; l<num_bands; l++) {
                ...
                double heat_l = - F0[l] * ((FR[1]*AR[1] - FR[0]*AR[0]) + (FP[1]*AP[1] - FP[0]*AP[0])) / dV(k,j,i);
                
                // Divide the flux over the different species
                double ktot = 0;
                for (int s=0; s < num_species; s++)
                    ktot += rho_all(s,k,j,i)*kappa(s,l) ;
...
void SphericalShortChar::dump_rays(std::ostream& f) const {
    rays->dump(f, data) ;
}
