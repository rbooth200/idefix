...
KOKKOS_INLINE_FUNCTION real RadiationFluxLimiterKley1989(real R){
  if (R<=2.0) return 2.0/(3.0+sqrt(9.0+10.0*R*R));
  else        return 10.0/(10.0*R+9.0+sqrt(81.0+180.0*R));  
}

KOKKOS_INLINE_FUNCTION real RadiationFluxLimiterMinerbo1978(real R){
  if (R<=1.5) return 2.0/(3.0+sqrt(9.0+12.0*R*R));
  else        return 1.0/(1.0+R+sqrt(1.0+2.0*R));
}
...
// evaluates the first derivative of the lagrange polynomial defined at (x1,y1), (x2,y2) at x to interpolate dy/dx
	real c1 = 1.0 / (x1-x2);
	real c2 = -c1;
  return c1*y1 + c2*y2;
}
...
FLDMatrix::FLDMatrix(Input& input, DataBlock* datain) {
  ...
  this->M = IdefixArray4D<real> ("Matrix",  2 * DIMENSIONS + 1,
                                            data->np_tot[KDIR],
                                            data->np_tot[JDIR],
                                            data->np_tot[IDIR]);
  ...
  if ((this->rbound[JDIR]==axis) || (this->lbound[JDIR]==axis)) {
    ...
    if(data->mygrid->nproc[KDIR]>1) {
      IDEFIX_ERROR("FLD:: Axis boundaries are not compatible with "
  ...
  #ifdef WITH_MPI
  this->arr4D = IdefixArray4D<real> ("WorkingArrayMpi", 1,  data->np_tot[KDIR],
                                                            data->np_tot[JDIR],
                                                            data->np_tot[IDIR]);
...
void FluxLimitedDiffusion::Init(Input &input, DataBlock *datain) {
  ...
  if(solver == BICGSTAB || solver == PBICGSTAB) {
    iterativeSolver = new Bicgstab<FLDMatrix>(*FLD_matrix.get(),
                                                targetError, maxiter,
                                                data->np_tot, data->beg, data->end);
  } else if(solver == CG || solver == PCG) {
    iterativeSolver = new Cg<FLDMatrix>(*FLD_matrix.get(),
                                                targetError, maxiter,
                                                data->np_tot, data->beg, data->end);
  } else if(solver == MINRES || solver == PMINRES) {
    iterativeSolver = new Minres<FLDMatrix>(*FLD_matrix.get(),
                                                targetError, maxiter,
                                                data->np_tot, data->beg, data->end);
  } else {
  ...
  this->rhs = IdefixArray3D<real> ("RadiationRHS",  data->np_tot[KDIR],
                                                    data->np_tot[JDIR],
                                                    data->np_tot[IDIR]);
                                                  
  this->Erad = IdefixArray3D<real> ("Erad", data->np_tot[KDIR],
                                            data->np_tot[JDIR],
                                            data->np_tot[IDIR]);
...
void FluxLimitedDiffusion::PreconditionMatrix() {
  ...
  idefix_for("PreconditionM", kbeg, kend, jbeg, jend, ibeg, iend,
  KOKKOS_LAMBDA (int k, int j, int i) {
    // can't set M to one because it might be negative!
    M(0,k,j,i) /= P(k,j,i) * P(k,j,i);
    D_EXPAND(
      M(1,k,j,i) /= P(k,j,i) * P(k,j,i-1);
...
void FluxLimitedDiffusion::PreconditionErad(bool undo) {
  ...
  if (not undo) {
    idefix_for("PreconditionE", kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
        Erad(k,j,i) *= P(k,j,i);
      });
 
  } else {
    idefix_for("UndoPreconditionE", kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
        Erad(k,j,i) /= P(k,j,i);
      });
  }
...
void FluxLimitedDiffusion::InitSolver() {
  ...
  int nanErad = 0;
  idefix_reduce("checkNanErad",0, data->np_tot[KDIR], 0, data->np_tot[JDIR], 0, data->np_tot[IDIR],
...
void FLDMatrix::operator()(IdefixArray3D<real> array, IdefixArray3D<real> laplacian) {
  ...
  idefix_for("FiniteDifference", kbeg, kend, jbeg, jend, ibeg, iend,
    KOKKOS_LAMBDA (int k, int j, int i) {
      ...
      real Delta = M(0,k,j,i) * array(k,j,i) + // line doesn't end here, keep adding...
      D_EXPAND(
        + M(1,k,j,i) * array(k,j,i-1) + M(2,k,j,i) * array(k,j,i+1),
...
void FLDMatrix::EnforceBoundary(int dir, BoundarySide side, RadiationBoundaryType type,
  ...
  switch(type) {
    ...
    case dirichlet: {
      ...
      idefix_for("BoundaryDirichlet", kbeg, kend, jbeg, jend, ibeg, iend,
            KOKKOS_LAMBDA (int k, int j, int i) {
              localVar(k,j,i) = val;
      });
      break;
    ...
    case neumann: {
      idefix_for("BoundaryNeumann", kbeg, kend, jbeg, jend, ibeg, iend,
            KOKKOS_LAMBDA (int k, int j, int i) {
...
void FluxLimitedDiffusion::EnrollUserDefOpacityCGS(PrototypeOpacityFuncCGS myFunc) {
  if (!haveUserDefOpacity) {
    IDEFIX_ERROR("FLD:: Attempt to enroll opacity function but a constant value has already been set");
  }
...
void TwoTemperatureFLD::Init(Input &input, DataBlock *datain) {
  ...
  this->radY = IdefixArray4D<real> ("RadiationY", num_species,
                                                  data->np_tot[KDIR],
                                                  data->np_tot[JDIR],
                                                  data->np_tot[IDIR]);
...
void TwoTemperatureFLD::FillUtils() {
  ...
  for (int s=0; s < num_species; s++) {
    idefix_for("FillUtils", kbeg, kend, jbeg, jend, ibeg, iend,
      KOKKOS_LAMBDA (int k, int j, int i) {
        real rho = Vc(RHO,k,j,i);
