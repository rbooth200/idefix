...
void TimeIntegrator::Cycle(DataBlock &data) {
  ...
  for(int stage=0; stage < nstages ; stage++) {
    ...
    // Update Uc & Vs
    // data.EvolveStage(); Skip hydro for this problem
    Kokkos::fence();
    computeLastLog += timer.seconds();
    ...
    if (data.CheckNan()>0) idfx::cout << stage << " Nan's found before stage loop\n" ;
    ...
    if (data.CheckNan()>0) idfx::cout << stage << " Nan's found after stage loop\n" ;
